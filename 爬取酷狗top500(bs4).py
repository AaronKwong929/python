...
def get_info(url): 
    ...
    ranks = soup.select(
        'div.pc_temp_songlist > ul > li > span.pc_temp_num '
    )  # rankWrap > div.pc_temp_songlist > ul > li:nth-child(3) > span.pc_temp_num > strong
    ...
    )  # 对歌手 - 歌名右键->检查->对弹出项COPY->SELECTOR 得到#rankWrap > div.pc_temp_songlist > ul > li:nth-child(1) > a
...
if __name__ == '__main__':
    urls = [
        'http://www.kugou.com/yy/rank/home/{}-8888.html'.format(str(i))
        for i in range(1, 24)
    ]  # 1到23页 每页22个 format 并将i转为字符串类型
    for url in urls:
        get_info(url)
